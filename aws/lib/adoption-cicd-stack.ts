...
export class CICDStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const repository = ecr.Repository.fromRepositoryName(
      this,
      'AdoptionECRFront',
      String(process.env.ECR_NAME_ADOPTION_NEXTJS)
    )
    ...
    const sourceRepo = codecommit.Repository.fromRepositoryName(
      ...
      String(process.env.CODECOMMIT_ADOPTION_NEXTJS)
    )
        ...
        phases: {
          pre_build: {
            commands: [
              ...
              `aws ecr get-login-password --region ${String(
                ...
                process.env.ECR_URI_ADOPTION_NEXTJS
              )}`,
          ...
          build: {
            commands: [
              ...
              `docker push ${String(process.env.ECR_URI_ADOPTION_NEXTJS)}`,
            ],
          },
          post_build: {
            commands: [
          ...
          actions: [
            new codepipeline_actions.EcsDeployAction({
              ...
              service: ecs.FargateService.fromFargateServiceAttributes(
                ...
                'AdoptionService',
                {
                  serviceArn: String(process.env.ECS_FRONT_ARN),
                  cluster,
                }
              ),
              input: buildOutput,
