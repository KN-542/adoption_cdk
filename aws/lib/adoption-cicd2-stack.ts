...
export class CICD2Stack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const repository = ecr.Repository.fromRepositoryName(
      this,
      'AdoptionECRFront',
      String(process.env.ECR_NAME_APPLICANT_NEXTJS)
    )
    ...
    const sourceRepo = codecommit.Repository.fromRepositoryName(
      ...
      String(process.env.CODECOMMIT_APPLICANT_NEXTJS)
    )
        ...
        phases: {
          pre_build: {
            commands: [
              ...
              `aws ecr get-login-password --region ${String(
                ...
                process.env.ECR_URI_ADOPTION_NEXTJS2
              )}`,
          ...
          build: {
            commands: [
              ...
              `docker push ${String(process.env.ECR_URI_ADOPTION_NEXTJS2)}`,
            ],
          },
          post_build: {
            commands: [
              ...
              `printf '[{"name":"${String(
                process.env.FRONT2_CONTAINER_NAME
              )}","imageUri":"${String(
                process.env.ECR_URI_ADOPTION_NEXTJS2
          ...
          actions: [
            new codepipeline_actions.EcsDeployAction({
              ...
              service: ecs.FargateService.fromFargateServiceAttributes(
                ...
                'AdoptionService',
                {
                  serviceArn: String(process.env.ECS_FRONT2_ARN),
                  cluster,
                }
              ),
              input: buildOutput,
